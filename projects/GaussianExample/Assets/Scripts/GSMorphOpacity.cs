...
public class GSMorphOpacity : MonoBehaviour
{
    ...
    private void Start() {
        ...
        GameObject currentGSObject = transform.GetChild(CurrentTargetIndex).gameObject;
    ...
    private void HandleGSAssetChange(int indexShift){
        ...
        if (CurrentTargetIndex + indexShift < 0 || CurrentTargetIndex + indexShift >= _numGSAssets || _isMorphing){
        ...
        _currentGSRenderer = transform.GetChild(CurrentTargetIndex).gameObject.GetComponent<GaussianSplatRenderer>();
        ...
        Debug.Log("new GS asset index: " + CurrentTargetIndex);
